import os
os.environ['OPENMC_CROSS_SECTIONS'] = '/Users/gregoiredupre/nndc_hdf5/cross_sections.xml'
...
for i in range (0,size-1):
    for j in range (0,size-1):
        if flux_f.mean[i][j]!=0 :
            flux_f.mean[i][j]=log(flux_f.mean[i][j])
    
for i in range (0,size-1):
    for j in range (0,size-1):
        if abs(flux_a.mean[i][j]) < 10**-6 :
            flux_a.mean[i][j]= 0
        elif abs(flux_a.mean[i][j])< 10**-4 :
            flux_a.mean[i][j]= 1
        elif abs(flux_a.mean[i][j])< 10**-3 :
            flux_a.mean[i][j]= 3
        elif abs(flux_a.mean[i][j])< 10**-2 : 
            flux_a.mean[i][j]= 5
        elif abs(flux_a.mean[i][j])< 10**-1 :
            flux_a.mean[i][j]= 7
        elif abs(flux_a.mean[i][j])< 10**-0 : 
            flux_a.mean[i][j]= 9
        else :
            flux_a.mean[i][j]= 20
...
fig = plt.subplot(121)
fig.imshow(flux_f.mean)
fig2 = plt.subplot(122)
fig2.imshow(flux_a.mean)
